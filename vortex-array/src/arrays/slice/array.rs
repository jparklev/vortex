...
impl Array<Slice> {
    ...
    pub fn new(child: ArrayRef, range: Range<usize>) -> Self {
        unsafe { Array::from_parts_unchecked(Self::new_parts(child, range)) }
    }
    ...
    /// Builds the [`ArrayParts<Slice>`] without bounds-checking the range. See
    /// [`Self::try_new_parts`] for the checked variant.
    pub fn new_parts(child: ArrayRef, range: Range<usize>) -> ArrayParts<Slice> {
        let len = range.len();
        let dtype = child.dtype().clone();
