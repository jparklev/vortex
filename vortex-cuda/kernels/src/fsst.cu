...
// (up to 8 bytes). Refills happen via the mirror of `Scratch::drain` on
// the input side: each step picks the largest naturally-aligned load
// that fits at the current fill position and in the remaining chunk
// capacity. CUDA u64 loads require natural 8-byte alignment, so reading
