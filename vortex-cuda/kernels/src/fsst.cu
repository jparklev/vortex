...
// (up to 8 bytes). Refills use the mirror of `Scratch::drain` on the
// input side: each step picks the largest naturally-aligned load that
// fits at the current fill position and in the remaining chunk capacity.
...
// hierarchical fill walks up to a u64 boundary with smaller aligned
// loads; once aligned and chunk is empty, every fill is a single u64
// load.
//
//   load    gate                                       ptx
//   ------  -----------------------------------------  ----------------
...
// changes the trade-off; the next commit re-stages it in shared memory.)
...
extern "C" __global__ void fsst(const uint8_t *__restrict codes_bytes,
    ...
    for (uint64_t sid = block_start + threadIdx.x; sid < block_end; sid += blockDim.x) {
        ...
        while (in_pos < in_end) {
            ...
            // Refill chunk via hierarchical aligned loads; see file header.
            if (chunk_bytes < 2) {
